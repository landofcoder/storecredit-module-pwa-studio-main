...
const SUPPORTED_PRODUCT_TYPES = ['SimpleProduct', 'ConfigurableProduct','StoreCreditProduct'];
 ...
 * @param {GraphQLQuery} props.addStoreCreditProductsToCartMutation - configurable product mutation
 * 
...
export const useProductFullDetail = props => {
    const {
        ...
        addStoreCreditProductsToCartMutation,
        product
    } = props;
    ...
    const [inputCredit, setInputCredit] = useState(product.min_credit)
    const handleInputCredit = useCallback(
        (e) => {
    ...
    const [
        addStoreCreditProductsToCart,{
            error: errorAddStoreCreditProductsToCart,
            loading: isAddStoreCreditLoading
        }
    ] = useMutation(addStoreCreditProductsToCartMutation)
    ...
    const handleAddToCart = useCallback(async () => {
        ...
        if (isSupportedProductType) {
            ...
            }else if(productType === 'StoreCreditProduct'){
                try {
                    await addStoreCreditProductsToCart({
                        variables
                    });
                } catch {
                    return;
    ...
    const derivedErrorMessage = useMemo(
        () =>
            deriveErrorMessage([
                ...
                errorAddStoreCreditProductsToCart,
            ]),
        [errorAddingConfigurableProduct, errorAddingSimpleProduct,errorAddStoreCreditProductsToCart]
    );
